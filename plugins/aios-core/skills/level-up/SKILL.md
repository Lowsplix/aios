...
מלא את השלבים מתוך עמודת "שלבים" של ההצעה שנבחרה. אם המיומנות צריכה לרוץ על לוח זמנים, הוסף סעיף `## תזמון` שמסביר איך לתזמן אותה (דרך מיומנות התזמון או cron) ומציין שהמחשב חייב להיות דלוק בזמן הריצה. אם היא נוגעת במערכת חיצונית, הוסף בלוק ```bash עם הפקודה האמיתית.
