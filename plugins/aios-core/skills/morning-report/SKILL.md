...
> [!warning] בדיקת מערכת הפעלה קודם. `wacli` רץ רק על macOS. במכונת Windows הוא לא קיים, וזה תקין. אל תיכשל בגלל זה. הבריף כבר נכתב להערת היום בשלב 3, וזו התוצאה החשובה. אם `wacli` לא זמין, דלג על השליחה, רשום `⚠️ חלקי` עם השורה "WhatsApp לא זמין במכונה הזו (wacli דורש macOS)", וציין בשורת בריאות המערכת שהבריף נכתב אך לא נשלח. ממשק הוואטסאפ דורש מכונת macOS או ממסר (relay), והוא מתוכנן כתוספת קרובה.

בדוק קודם אם `wacli` קיים ועובד. אם הפקודה לא נמצאת, אנחנו על Windows (או שהכלי לא הותקן): דלג על כל שלב 4 והמשך לשלב 5 עם סטטוס חלקי.

```bash
# שער מערכת הפעלה + שער חיות: בלי שניהם לא שולחים
if ! command -v wacli >/dev/null 2>&1; then
  echo "⚠️ wacli לא קיים במכונה הזו (דורש macOS). הבריף נכתב לכספת, לא נשלח בוואטסאפ."
else
  wacli doctor
fi
```
